...
export default function friendsRouter(
  ...
  router.get("", async (req, res): Promise<void> => {
    try {
      ...
      const friendships: Friendship[] | null = await fetchFriendships(
        req.user?.id!,
      );
  ...
  router.post("/request", async (req, res): Promise<void> => {
    ...
    try {
      ...
      const checkExistingFriendship = await getFriendship(
      ...
      if (checkExistingFriendship) {
        res
          ...
          .json({
            error:
              "Cannot send a friend request to someone you're already friends with",
          });
        return;
      }
      ...
      const checkExistingFriendRequest =
        await getFriendRequestBySenderAndReceiver(
          friendRequest.sender_id,
          friendRequest.receiver_id,
        );
      if (checkExistingFriendRequest) {
        res
          ...
          .json({
            error:
              "Cannot send a friend request if there is already a pendig friend request to the same person",
          });
        return;
      }
      ...
      if (reciverSockets) {
        for (const socket of reciverSockets)
          io.to(socket).emit("friend_request_received", request);
      }
  ...
  router.post("/request/accept", async (req, res) => {
    ...
    try {
      ...
      const friendRequest: FriendRequestInput | null =
        await getFriendRequestById(req.body.id);
      if (!friendRequest) {
